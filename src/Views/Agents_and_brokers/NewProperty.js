...
function NewProperty() {
    ...
    const [picker,setPicker] = useState({
        owners_pool:["Alpha Lagatt","James Misoy","Hardika Pareek","Ankit Acharya","Gilbert Koech"],
        selected_owners:[],
        searched_owners:[],
        agents_pool:["John Mwangi","Ian Kipruto","John Doe","Jimroni Keter","Dennis Seroney","Collins Kipchirchir Biwott"],
        selected_agents:[],
        searched_agents:[],
        pickedVal:"",
        ownersPickedVal:"",
    ...
    const searchAgentArray = (e)=>{
        ...
        setPicker((prev)=>{
            return {...prev,
               searched_agents : picker.agents_pool.filter(myList=>myList.includes(text)),
               pickedVal:text
            }
        })
    }
    ...
    const searchOwnersArray = (e)=>{
        ...
        setPicker((prev)=>{
            return {...prev,
               searched_owners : picker.owners_pool.filter(myList=>myList.includes(text)),
    ...
    <div  className="form-container">
        ...
        {({ getInputProps, suggestions, getSuggestionItemProps, loading }) => (
          <div>
            <input {...getInputProps({ placeholder: 'Search Address ...', className: 'location-search-input',})} />
        ...
        <div className="form-text-area">
            <textarea ref={description_ref} id="property-description" type="textarea" rows={8} cols={50} placeholder="Property Description" />
        </div>
        </div>

        <div>
            <div className="form-sub-section-heading"><h4>Property Characteristics</h4></div>
            <div className="form-characteristics-container">
                <div className="form-characteristics">
                    <label>Bedrooms</label><br/>
                    <input ref={bedrooms_ref} type="Number"/>
                </div>
                <div className="form-characteristics">
                    <label>Bathrooms</label><br/>
                    <input ref={bathrooms_ref} type="Number"/>
                </div>
                <div className="form-characteristics">
                    <label>CarPort</label><br/>
                    <input ref={carport_ref} type="Number"/>
                </div>
            </div>
        </div>

        <div >
            <div className="form-sub-section-heading"><h4>Managing Agents</h4></div>
            <div className="form-picker-cont">
                <div className="form-picker">
                    <input type="text" onChange={searchAgentArray} value={picker.pickedVal} placeholder="Search Agents.." />
                    <div className="form-picker-list">
                        {picker.searched_agents.map((agent)=>{
                            return <div className="form-picker-item" onClick={()=>{
                                return addToSelected(agent);
                            }}>{agent}</div>
                        })}
                    </div>
                </div>
                <div className="form-picker-picked-container" >{picker.selected_agents.map((item)=>{
                    return <div className="form-picker-picked">{item} <span><img onClick={()=>{return removeToSelected(item)}} src={require("../../RESOURCES/close.png")} alt="X" /></span></div>
                })}
        ...
        <div >
            <div className="form-sub-section-heading"><h4>Owners Information</h4></div>
            <div className="form-picker-cont">
                <div className="form-picker">
                    <input type="text" onChange={searchOwnersArray} value={picker.ownersPickedVal} placeholder="Search Owners.." />
                    <div className="form-picker-list">
                ...
                <div className="form-picker-picked-container" >{picker.selected_owners.map((item)=>{
                    return <div className="form-picker-picked-owner">{item} <span><img onClick={()=>{return Owners_removeToSelected(item)}} src={require("../../RESOURCES/close.png")} alt="X" /></span></div>
                })}
        ...
        <div>
        <div className="form-sub-section-heading"><h4>Property Designation</h4></div>
            <div className="form-radio-group">
                <div className="form-radio-group-cont">
                    <div className={picker.forSale?"form-radio-group-item1-selected":"form-radio-group-item1"} onClick={toggleForSale}>
                    </div> 
                    <div>For Sale</div>
                </div>
                <div hidden={!picker.forSale}>
                    Price of The Property: <input id="property-address" type="Number" placeholder="Marked Price.." />
                </div>
                
            </div>
            <div className="form-radio-group">
                <div  className="form-radio-group-cont">
                    <div className={!picker.forSale?"form-radio-group-item2-selected":"form-radio-group-item2"} onClick={toggleForRent}>
                    </div> 
                    <div>For Rent/Lease</div>
                </div>
                <div hidden={picker.forSale}>
                    Rent Per Month: <input id="property-address" type="text" placeholder="Marked Price or Rent Per Month" />
                </div>
            </div>
        </div>
        ...
        <div>
        <div className="form-sub-section-heading"><h4>Property Images</h4></div>
                        ...
                        {({getRootProps,getInputProps,isDragActive,isDragAccept,isDragReject})=>{
                            ...
                            return(
                                <div {...getRootProps({className:`dropzone ${additionalClass}`,})}>
                                    <input {...getInputProps()} />
                                    <img src={require("../../RESOURCES/cloud_upload.png")} alt="upload" />
    ...
    <input type="submit" onClick={submit_prop} value="submit"/>
    </div>
}
