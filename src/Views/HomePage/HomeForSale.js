    ...
    <div className="home-section-two-forsale-carousel">
        { ForSaleProperties.tieredResults[0].results.map((property)=>{
            ...
            <div className="home-section-two-forsale-footer">
                { (pageSize === '4') && <Link to={"/home-properties/forsale/12/1/"+address} className="home-section-two-forsale-footer-btn">VIEW ALL FOR SALE</Link>}
                { (pageSize === '12') && <div>
                     <Link to={"/home-properties/forsale/12/1/"+address} className="home-section-two-forsale-footer-btn">1</Link>
                     <Link to={"/home-properties/forsale/12/2/"+address} className="home-section-two-forsale-footer-btn">2</Link>
                     <Link to={"/home-properties/forsale/12/3/"+address} className="home-section-two-forsale-footer-btn">3</Link>
                     <Link to={"/home-properties/forsale/12/4/"+address} className="home-section-two-forsale-footer-btn">4</Link>
                     <Link to={"/home-properties/forsale/12/5/"+address} className="home-section-two-forsale-footer-btn">5</Link>
                </div>}
            </div>
            </>
  )
...
export const HomePropertiesForSaleLoader = async ({params}) => {
    const pagesize = params.pageSize;
    const pageNumber = params.pageNumber;
    const address = params.address;
    const url = 'https://realty-in-au.p.rapidapi.com/properties/list?channel=buy&searchLocation='+address+'&searchLocationSubtext=Region&type=region&page='+pageNumber+'&pageSize='+pagesize+'&sortType=relevance&surroundingSuburbs=true&ex-under-contract=false';
    const options = {
        method: 'GET',
        headers: {
